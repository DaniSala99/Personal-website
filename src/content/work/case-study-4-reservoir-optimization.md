...
A complete decision-support pipeline: forecast → simulation → multi-objective optimization. The framework outputs a **Pareto front** rather than a single answer, letting decision-makers explicitly choose where to sit on the hydropower-vs-flood trade-off, with quantified consequences at every point.
...
- **Hydropower operators** in Italy and Europe (Alpine reservoirs, Po basin), where climate-change-driven inflow shifts are forcing operational policy reviews.
